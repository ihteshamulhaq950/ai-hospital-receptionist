...
RULES
1. Non-English input → translate to English. Always output refinedQuery in English.
2. needsRAG=true → rewrite refinedQuery: expand abbreviations (OPD→Outpatient Department), use formal hospital terminology, fix grammar.
3. complex_query → populate subQueries with individual English questions.
4. medical_guidance → triggered by symptoms or "which doctor for X". Return answer + exactly 5 suggestions. Set refinedQuery="" and subQueries=[].
5. All other intents → return answer="" and suggestions=[].

SPECIALIST MAPPINGS (for medical_guidance)
...
async function classifyQuery(userQuery: string): Promise<ClassifiedQuery> {
  try {
    ...
    console.log("[WA RAG] Raw classifier result:", JSON.stringify(result));
    ...
    console.log("[WA RAG] Classified:", JSON.stringify(classified));
    return classified;

  } catch (err) {
    console.error("[WA RAG] classifyQuery threw:", err);
...
Use ONLY the context below to answer. Be concise (max 5 lines). Plain text only, no markdown.

Context: ${context.slice(0, 1200)}

Question: ${query}

Return STRICT JSON only: { "answer": "...", "suggestions": ["5 short follow-up questions"] }`
    : `You are a hospital information assistant replying via WhatsApp.
No information found. Politely say so in 1 sentence. Suggest asking about timings, departments, appointments, or location.

Question: ${query}

Return STRICT JSON only: { "answer": "...", "suggestions": ["5 alternative questions"] }`;
...
export async function getWhatsAppAnswer(userMessage: string): Promise<WhatsAppRAGResult> {
  ...
  try {
    ...
    if (classified.intent === "medical_guidance") {
      return {
        answer: classified.answer?.trim() || FALLBACK_ANSWER,
...
//     const context = await searchPinecone(queries, 4);
//     console.log("[WA RAG] Context chars:", context.length);
