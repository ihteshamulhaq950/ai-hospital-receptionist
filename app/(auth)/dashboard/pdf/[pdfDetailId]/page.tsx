...
export default function PDFDetailPage({
  ...
  params: Promise<{ pdfDetailId: string }>;
}) {
  ...
  const fetchPDFDetails = async () => {
    try {
      ...
      const response = await fetch(
        `/api/dashboard/pdf/${resolvedParams.pdfDetailId}`,
      );
  ...
  const handleCleanupStorageAndDB = async () => {
    if (
      !confirm(
        "Are you sure? This will permanently delete the file and database record.",
      )
    ) {
      return;
    }
  ...
  const handleCleanupDatabase = async () => {
    if (
      !confirm(
        "Are you sure? This will permanently delete the database record.",
      )
    ) {
      return;
    }
  ...
  const handleDownload = async () => {
    ...
    if (!pdf || !pdf.uploadedFilePath) return;

    try {
      const response = await fetch("/api/dashboard/pdf/download", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          storagePath: pdf.uploadedFilePath,
          fileName: pdf.name,
          download: true,
        }),
      });
    ...
    <div className="space-y-6">
      ...
      {pdf.embeddedStatus === "deleted" && (
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            <strong>Warning:</strong> This document is no longer available in
            RAG search. Embeddings have been deleted from the vector database.
            Please delete this document to complete cleanup.
          </AlertDescription>
        </Alert>
      )}
...
//               variant="outline"
//               className="w-full bg-transparent"
//               onClick={handleOpenFile} // Changed from asChild
//               disabled={pdf.embeddedStatus === "deleted"}
//             >
//               <ExternalLink className="mr-2 h-4 w-4" />
//               Open Original File
