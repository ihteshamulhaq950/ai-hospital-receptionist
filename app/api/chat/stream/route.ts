  ...
  (async () => {
    try {
      ...
      if (assistantMsgError) {
        console.error("[Stream API] Failed to save assistant message:", assistantMsgError);
        sendEvent("error", { message: "Failed to save response" });
        close();
        return;
      }

      console.log("[Stream API] Assistant message saved:", assistantMessage.id);

      // Increment message count (fire & forget)
      supabase
        .rpc("increment_message_count", { session_id: chatSessionId })
        .then(({ error }) => {
